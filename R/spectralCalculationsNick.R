...
calculateMeanRows <- function(normalized) {
  averageRow <- function(rn, r) {
    a <- apply(terra::values(r, rn, 1), 2, mean)
  }
...
rasterRABD <- function(normalized,
  ...
  l <- terra::subset(normalized$normalized, li)
  if (length(li) > 1) {
  ...
  h <- terra::subset(normalized$normalized, hi)
  if (length(hi) > 1) {
    if (edge.agg.fun == "mean") {
      h <- mean(h)
    } else if (edge.agg.fun == "min") {
  ...
  m <- terra::subset(normalized$normalized, mi)
  if (length(mi) > 1) {
